function napi_create_array (env: napi_env, result: Pointer<napi_value>): napi_status {
  return emnapiCtx.checkEnv(env, (envObject) => {
    return emnapiCtx.checkArgs(envObject, [result], () => {
      ...
      const value = emnapiCtx.addToCurrentScope([])
...
function napi_create_array_with_length (env: napi_env, length: size_t, result: Pointer<napi_value>): napi_status {
  return emnapiCtx.checkEnv(env, (envObject) => {
    return emnapiCtx.checkArgs(envObject, [result], () => {
      ...
      // @ts-expect-error
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const value = emnapiCtx.addToCurrentScope(new Array(length))
      $makeSetValue('result', 0, 'value', '*')
      return envObject.clearLastError()
    })
  })
...
function napi_create_arraybuffer (env: napi_env, byte_length: size_t, _data: void_pp, result: Pointer<napi_value>): napi_status {
  return emnapiCtx.preamble(env, (envObject) => {
    return emnapiCtx.checkArgs(envObject, [result], () => {
      ...
      // @ts-expect-error
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const value = emnapiCtx.addToCurrentScope(new ArrayBuffer(byte_length))
      $makeSetValue('result', 0, 'value', '*')
...
function napi_create_date (env: napi_env, time: double, result: Pointer<napi_value>): napi_status {
  return emnapiCtx.preamble(env, (envObject) => {
    return emnapiCtx.checkArgs(envObject, [result], () => {
      ...
      // @ts-expect-error
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const value = emnapiCtx.addToCurrentScope(new Date(time))
      $makeSetValue('result', 0, 'value', '*')
      return envObject.getReturnStatus()
    })
  })
...
function napi_create_external (env: napi_env, data: void_p, finalize_cb: napi_finalize, finalize_hint: void_p, result: Pointer<napi_value>): napi_status {
  return emnapiCtx.preamble(env, (envObject) => {
    return emnapiCtx.checkArgs(envObject, [result], () => {
      ...
      const externalHandleId = emnapiCtx.getCurrentScope()!.addExternal(data)
      if (finalize_cb) {
        emnapiRt.Reference.create(envObject, externalHandleId, 0, emnapiRt.Ownership.kRuntime, finalize_cb, data, finalize_hint)
      }
      $from64('result')
      $makeSetValue('result', 0, 'externalHandleId', '*')
      return envObject.clearLastError()
    })
  })
}
  ...
  return emnapiCtx.checkEnv(env, (envObject) => {
    return emnapiCtx.checkArgs(envObject, [result], () => {
      ...
      if (!description) {
        ...
        const value = emnapiCtx.addToCurrentScope(Symbol())
        $makeSetValue('result', 0, 'value', '*')
      } else {
        ...
        // @ts-expect-error
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const v = emnapiCtx.addToCurrentScope(Symbol(desc))
  ...
  return emnapiCtx.preamble(env, (envObject) => {
    return emnapiCtx.checkArgs(envObject, [arraybuffer, result], () => {
      ...
      const retCallback = (out: ArrayBufferView): napi_status => {
        ...
        // @ts-expect-error
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const value = emnapiCtx.addToCurrentScope(out)
        $makeSetValue('result', 0, 'value', '*')
        return envObject.getReturnStatus()
      }
...
function napi_create_dataview (
  ...
  return emnapiCtx.preamble(env, (envObject) => {
    return emnapiCtx.checkArgs(envObject, [arraybuffer, result], () => {
      ...
      // @ts-expect-error
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const value = emnapiCtx.addToCurrentScope(dataview)
      $makeSetValue('result', 0, 'value', '*')
      return envObject.getReturnStatus()
    })
  })
...
function node_api_symbol_for (env: napi_env, utf8description: const_char_p, length: size_t, result: Pointer<napi_value>): napi_status {
  return emnapiCtx.checkEnv(env, (envObject) => {
    return emnapiCtx.checkArgs(envObject, [result], () => {
      ...
      // @ts-expect-error
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const value = emnapiCtx.addToCurrentScope(Symbol.for(descriptionString))
      $makeSetValue('result', 0, 'value', '*')
      return envObject.clearLastError()
    })
  })
