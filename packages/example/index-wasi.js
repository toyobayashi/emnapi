import * as emnapi from './node_modules/@emnapi/runtime/dist/emnapi.js'
...
async function mainThread (url, emnapiCtx) {
...
async function main () {
  ...
  if (!isChildThread) {
    const emnapiCtx = emnapi.createContext()
    const { instance, napiModule } = await mainThread(new URL('./out/wasi-sdk/binding.wasm', import.meta.url), emnapiCtx)
