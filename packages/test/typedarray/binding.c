...
static napi_value Multiply(napi_env env, napi_callback_info info) {
  ...
  NODE_API_ASSERT(env, address == data, "input_buffer wrong address");
  NODE_API_ASSERT(env, ownership == (support_weakref ? emnapi_runtime : emnapi_userland), "input_buffer wrong ownership");
  NODE_API_ASSERT(env, runtime_allocated, "input_buffer wrong runtime_allocated");
  emnapi_get_memory_address(env, output_buffer, &address, &ownership, &runtime_allocated);
  NODE_API_ASSERT(env, address == output_ptr, "output_buffer wrong address");
  NODE_API_ASSERT(env, ownership == (support_weakref ? emnapi_runtime : emnapi_userland), "input_buffer wrong ownership");
  NODE_API_ASSERT(env, runtime_allocated, "output_buffer wrong runtime_allocated");
#endif
