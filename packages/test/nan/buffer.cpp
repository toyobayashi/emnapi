...
NAN_METHOD(New1) {
  ...
  for (unsigned char i = 0; i < DATA_SIZE; i++) dynamic_data[i] = 'a' + i;
...
NAN_METHOD(New2) {
  ...
  for (unsigned char i = 0; i < DATA_SIZE; i++) pbuf[i] = 'a' + i;
...
NAN_MODULE_INIT(Init) {
  for (unsigned char i = 0; i < DATA_SIZE; i++) data[i] = 'a' + i;
  Set(target, New<v8::String>("new1").ToLocalChecked(),
      GetFunction(New<v8::FunctionTemplate>(New1)).ToLocalChecked());
  Set(target, New<v8::String>("new2").ToLocalChecked(),
      GetFunction(New<v8::FunctionTemplate>(New2)).ToLocalChecked());
  Set(target, New<v8::String>("new3").ToLocalChecked(),
      GetFunction(New<v8::FunctionTemplate>(New3)).ToLocalChecked());
  Set(target, New<v8::String>("copy").ToLocalChecked(),
