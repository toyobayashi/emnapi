...
static napi_value _create_symbol(napi_env env, napi_callback_info info) {
  ...
  napi_create_string_utf8(env, "symbol", -1, &description);
  napi_create_symbol(env, description, &ret);
  return ret;
}
...
NAPI_MODULE_INIT() {
  ...
  napi_create_function(env, NULL, 0, _create_array_with_length, NULL, &js_create_array_with_length);
